...
const maxCopies = (c: any) => (c.rarity === 'legendary' || c.rarity === 'mythic') ? 1 : 2
...
export function RealDeckBuilder() {
  ...
  // Carica mazzo salvato
  useEffect(() => {
    try {
      const raw = localStorage.getItem(DECK_KEY)
      if (raw) { const d = JSON.parse(raw); if (d.counts) setDeck(d.counts); if (d.name) setDeckName(d.name) }
    } catch {}
  }, [])
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', height: 'calc(100vh - 64px)', overflow: 'hidden', background: '#06080f' }}>
      {/* Catalogo */}
      <div style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden', borderRight: '1px solid rgba(240,165,0,0.08)' }}>
        <div style={{ padding: '16px 24px', borderBottom: '1px solid rgba(240,165,0,0.08)', background: 'rgba(6,8,15,0.9)', display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div>
            <div className="font-cinzel" style={{ fontSize: 18, fontWeight: 700, color: '#e8dcc8', letterSpacing: '0.06em' }}>Le Tue Carte</div>
            <div style={{ fontSize: 11, color: 'rgba(232,220,200,0.4)', marginTop: 2 }}>{cards.length} carte create · clicca per aggiungere al mazzo</div>
          </div>
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            ...
            {types.length > 1 && (
              <select value={fType} onChange={e => setFType(e.target.value)} style={{ padding: '8px 12px', background: 'rgba(13,17,32,0.8)', border: '1px solid rgba(240,165,0,0.15)', borderRadius: 8, color: 'rgba(232,220,200,0.8)', fontSize: 11, outline: 'none', cursor: 'pointer' }}>
                <option value="tutti">Tutti i tipi</option>
                {types.map(t => <option key={t} value={t}>{String(t).toLowerCase()}</option>)}
              </select>
            )}
          </div>
        </div>
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px', display: 'flex', flexWrap: 'wrap', gap: 14, alignContent: 'flex-start' }}>
          {loaded && cards.length === 0 && (
            <div style={{ width: '100%', padding: '60px 20px', textAlign: 'center', color: 'rgba(232,220,200,0.35)', fontSize: 13, lineHeight: 1.7 }}>
              ...
              Nessuna carta creata. Apri il <strong style={{ color: '#f0a500' }}>Card Creator Studio</strong>, crea e pubblica le tue carte.
            </div>
          )}
          {filtered.map(card => {
            const cnt = deck[card.id] ?? 0
            return <RealCardTile key={card.id} card={card} onClick={() => add(card)} dimmed={cnt >= maxCopies(card)} badge={cnt > 0 ? `x${cnt}` : undefined} />
          })}
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', background: 'rgba(8,10,18,0.95)', overflow: 'hidden' }}>
        <div style={{ padding: '16px 20px', borderBottom: '1px solid rgba(240,165,0,0.08)' }}>
          ...
          <div style={{ marginTop: 12, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ fontSize: 12, color: 'rgba(232,220,200,0.6)' }}><span style={{ color: '#f0a500', fontWeight: 600 }}>{total}</span><span style={{ color: 'rgba(232,220,200,0.3)' }}> / 30 carte</span></div>
            ...
            return (
              <div key={card.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px', borderRadius: 8, marginBottom: 4, background: 'rgba(255,255,255,0.02)' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div className="font-cinzel" style={{ fontSize: 10, fontWeight: 600, color: accent, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{card.name}</div>
                  <div style={{ fontSize: 8, color: rar.color, marginTop: 1 }}>{rar.label}</div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
                  <button onClick={() => remove(card.id)} style={{ width: 18, height: 18, borderRadius: '50%', background: 'rgba(248,113,113,0.15)', border: '1px solid rgba(248,113,113,0.3)', color: '#f87171', fontSize: 14, lineHeight: 1, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>−</button>
                  <span className="font-cinzel" style={{ fontSize: 11, fontWeight: 700, color: '#f0a500', minWidth: 14, textAlign: 'center' }}>×{deck[card.id]}</span>
                  <button onClick={() => add(card)} disabled={(deck[card.id] ?? 0) >= maxCopies(card)} style={{ width: 18, height: 18, borderRadius: '50%', background: (deck[card.id] ?? 0) >= maxCopies(card) ? 'rgba(255,255,255,0.05)' : 'rgba(74,222,128,0.15)', border: `1px solid ${(deck[card.id] ?? 0) >= maxCopies(card) ? 'rgba(255,255,255,0.1)' : 'rgba(74,222,128,0.3)'}`, color: (deck[card.id] ?? 0) >= maxCopies(card) ? 'rgba(255,255,255,0.2)' : '#4ade80', fontSize: 14, lineHeight: 1, cursor: (deck[card.id] ?? 0) >= maxCopies(card) ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>+</button>
                </div>
              </div>
            )
          })}
        ...
        <div style={{ padding: '12px 20px', borderTop: '1px solid rgba(240,165,0,0.08)' }}>
          <button onClick={saveDeck} disabled={total === 0} className="btn-primary font-cinzel"
